...
The transcript is cached once per inspection (it's the largest stable
content), and each of the 4 extraction passes reads against that cached
prefix with a different focused system prompt.
"""
...
def require_api_key() -> None:
    if not os.environ.get("ANTHROPIC_API_KEY"):
        raise RuntimeError(
            "ANTHROPIC_API_KEY is not set. Export it before running real inspections."
        )
