...
The taste profile + system rubric are stable across every artist scored in
one planning run, so they're cache-friendly. Each artist annotation is a
fresh user turn against the same cached prefix.
"""
...
def require_api_key() -> None:
    if not os.environ.get("ANTHROPIC_API_KEY"):
        raise RuntimeError(
            "ANTHROPIC_API_KEY is not set. Export it before running a real plan."
        )
